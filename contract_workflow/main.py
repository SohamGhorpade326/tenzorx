...
# Make sure project root is on path so absolute imports like 'contract_workflow.graph...' work
...
# ── App setup ─────────────────────────────────────────────────────
app = FastAPI(
    title="Contract Workflow API",
    description="Agentic contract workflow built with LangGraph.",
    version="1.0.0",
)
...
async def health():
    return {"status": "ok", "service": "contract-api"}


@app.get("/", tags=["system"])
async def root():
    return {
        "service": "Contract Workflow API",
        "docs": "/docs",
        "health": "/health",
    }
