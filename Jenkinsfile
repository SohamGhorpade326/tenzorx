        ...
        stage('Build') {
            steps {
                ...
                script {
                    sh '''
                        # Build frontend
                        docker build -t workstream-ai-frontend:${BUILD_NUMBER} ./Workstream-AI/frontend
                        
                        # Build meeting workflow
                        docker build -t workstream-ai-meeting-workflow:${BUILD_NUMBER} ./Workstream-AI/microservices/meetingworkflow
                        
                        # Build video onboarding
                        docker build -t workstream-ai-video-onboarding:${BUILD_NUMBER} ./Workstream-AI/microservices/video_onboarding_service
                    '''
                }
            }
        }
                ...
                script {
                    sh '''
                        ...
                        ./sonar-scanner-4.10.0.2635-linux/bin/sonar-scanner \
                          ...
                          -Dsonar.sources=Workstream-AI/frontend/src,Workstream-AI/microservices \
