...
type RunWithLogCount = contractApi.RunSummary & { logs_count: number };
...
export default function ContractAnalytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true);
      const runList = await contractApi.listRuns(50);

      const detailed = await Promise.all(
        runList.runs.map(async (run) => {
          try {
            const detail = await contractApi.getRun(run.run_id);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-100">Contract Analytics Summary</h1>
